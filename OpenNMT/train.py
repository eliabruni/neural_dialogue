...
def main():
    ...
    if opt.train_from is None:
        encoder = onmt.OriginalModels.Encoder(opt, dicts['src'])
        decoder = onmt.OriginalModels.Decoder(opt, dicts['tgt'])
        generator = nn.Sequential(
            nn.Linear(opt.rnn_size, dicts['tgt'].size()),
            nn.LogSoftmax())
        if opt.cuda > 1:
            generator = nn.DataParallel(generator, device_ids=opt.gpus)
        model = onmt.OriginalModels.NMTModel(encoder, decoder, generator)
        if opt.cuda > 1:
